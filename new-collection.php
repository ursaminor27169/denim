...
<body>
    <?php
        ...
        $result = mysqli_query($link, "SELECT * FROM `new-collection`");
        $number = mysqli_query($link, "SELECT count(id) FROM `new-collection`");
        $ids = [];
        $titles = [];
        $imgs = [];
        $texts = [];
        $prices = [];
        $i = 0;
        while($newcollection = mysqli_fetch_assoc($result)) 
        {
            $ids[$i] = $newcollection['id'];
            $titles[$i] = $newcollection['title'];
            $imgs[$i] = $newcollection['img'];
            $texts[$i] = $newcollection['text'];
            $prices[$i] = $newcollection['price'];
            $i = $i + 1;
        }
    ?>
    <header>
        ...
        <div class="assortments">
            <?php
                ...
                while ($i < count($titles)) {
                    echo
                    '<a href="/new-collection-item.php/?id='. $ids[$i] .'" class="assortments-unit wow fadeInUp">
